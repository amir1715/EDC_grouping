...
cas_ids<-as.character(unique(cas_ids)[!unique(cas_ids) %in% ''])
...
path_to_EDCmet_phase<-'/research/groups/fortino/amirhs/EDCmet_new'
load(paste(path_to_EDCmet_phase,'/outputData/glm/integrated_fgsea_allcompounds_results_final_moa.RData',sep=''))
...
fgs<-fgs[names(fgs)[-grep(x=names(fgs),pattern=remove_pattern)]] # removing 9 TG-Gates networks
comp_mesh_fgs<-Reduce('intersect',lapply(fgs, function(x)rownames(x$x))) # the compounds in all layers

RDkit_properties<-read.csv('scripts/python/RD_kit_res.csv',stringsAsFactors = F) # calculated descriptors by RDkit
...
# Preselection of the pathways with GLM-coef and average NES scores
load(paste(path_to_EDCmet_phase,'/outputData/glm/glm_models_final_moa.RData',sep = ''))
modls<-modls[names(modls)[-grep(x=names(modls),pattern=remove_pattern)]]
if(all(names(modls)==names(pathway_scores))){
  temp_patw_scores<-list()
      for (i in 1:length(pathway_scores)){
    sel_by_glm<-rownames(modls[[i]]$cof)[which(modls[[i]]$cof > 0)] #GLM-coefs
    sel_by_NES<-names(apply(pathway_scores[[i]], 2, mean))[which(apply(pathway_scores[[i]], 2, mean)>.1)] #NES
    temp_patw_scores[[i]]<-pathway_scores[[i]][,intersect(sel_by_glm,sel_by_NES)]
  } # end for
}# end if
names(temp_patw_scores)<-names(pathway_scores)
print(sapply(temp_patw_scores, function(x)ncol(x)))
pathway_scores<-temp_patw_scores
...
dx_sclaed_pat_scores<-lapply(pathway_scores,function(x)(1-abs(cor(t(x), use="pairwise.complete.obs", method="pearson"))))
...
for (i in 1:length(pat_cluster)){
  ...
  names(grp[[i]])<-rownames(pathway_scores[[i]])
}
names(grp)<-names(pat_cluster)
save(dx_sclaed_pat_scores,pathway_scores,grp,file = 'outputData/patway_based_grouping.RData')
...
library(ggpubr)
pdf('outputData/plots/single_view_clust_pathways.pdf',width = 30,height = 30)
plot(ggarrange(plotlist = dend_plt_list,nrow = 5,ncol = 3,common.legend = T)  )
dev.off()
...
library(factoextra)
 for (i in 1:length(pathway_scores)){
  # rownames(scaled_pat_scores[[i]])<-names(grp[[i]])<-comp_names
   pl<-fviz_cluster(list(data = pathway_scores[[i]],
   cluster = grp[[i]]),main=names(pathway_scores)[[i]])
...
names(grp_mvc)<- rownames(pathway_scores$Drug_Matrix_Rat_invitro_Single_Dose_1_day)
