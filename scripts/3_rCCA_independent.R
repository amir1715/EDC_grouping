...
save(final_resuls,CV_resuls,file = 'outputData/independent_cvCCA.RData')
...
load('outputData/independent_cvCCA.RData')
...
save(gr_list,cor_mat,file = 'outputData/independent_graphs_CCA.RData')

library(ggpubr)
pdf('outputData/plots/independent_CCA.pdf',width = 40,height = 40)
...
library(circlize)
library(ggplotify)
load('outputData/independent_graphs_CCA.RData')
source('functions/general_function.R')

most_related_desc_mat<-lapply(cor_mat, function(x)most_related_desc(x))
most_related_desc_mat<-most_related_desc_mat[!names(most_related_desc_mat) %in% "Consensus_Rat_invitro_DM_TG_GATEs"]
circ_plot<-list()
...
pdf('outputData/plots/CCA_independentCircos.pdf',width = 30,height = 30)
